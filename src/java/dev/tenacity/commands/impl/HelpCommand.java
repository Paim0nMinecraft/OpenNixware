...
public class HelpCommand extends Command {
    ...
    public void execute(String[] args) {
        ChatComponentText response = new ChatComponentText("\n§l§dTenacity §r§d" + NIXWARE.VERSION + "§l " + NIXWARE.RELEASE.getName() + " §7- §r§6Hover to see command usages.");
        ChatComponentText temp = new ChatComponentText("");
        float maxLength = 0;
        for (Command cmd : NIXWARE.INSTANCE.getCommandHandler().getCommands()) {
