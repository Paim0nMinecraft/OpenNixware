...
public class Module extends ListenerAdapter implements Utils {
    ...
    public final String getSplicedName() {
        ...
        if(!ArrayListMod.splicedName.isEnabled()){
            return name;
        }

        if (this.splicedName.isEmpty()) {
            StringBuilder sb = new StringBuilder();
            char[] arr = name.toCharArray();

            char char1;
            for (int j = 0; j < arr.length; sb.append(char1)) {
                int i = j++;
                char1 = arr[i];
                if (i != 0 && !Character.isLowerCase(char1) && Character.isLowerCase(arr[i - 1])) {
                    sb.append(' ');
                }
            }

            splicedName = sb.toString();
        }
    ...
    public void onEnable() {
        NIXWARE.INSTANCE.getEventProtocol().register(this);
    }

    public void onDisable() {
        if (this instanceof GlowESP) {
            GlowESP.fadeIn.setDirection(Direction.BACKWARDS);
            Multithreading.schedule(() -> NIXWARE.INSTANCE.getEventProtocol().unregister(this), 250, TimeUnit.MILLISECONDS);
        } else {
            NIXWARE.INSTANCE.getEventProtocol().unregister(this);
        }
    }
