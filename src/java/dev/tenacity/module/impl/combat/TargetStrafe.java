...
public final class TargetStrafe extends Module {
    ...
    public static boolean canStrafe() {
        KillAura killAura = NIXWARE.INSTANCE.getModuleCollection().getModule(KillAura.class);
        if (!NIXWARE.INSTANCE.isEnabled(TargetStrafe.class) || !killAura.isEnabled()
                || !MovementUtils.isMoving() || (space.isEnabled() && !Keyboard.isKeyDown(Keyboard.KEY_SPACE))) {
            return false;
        }
        if (!(NIXWARE.INSTANCE.isEnabled(Speed.class) || NIXWARE.INSTANCE.isEnabled(Flight.class))) {
            return false;
        }
