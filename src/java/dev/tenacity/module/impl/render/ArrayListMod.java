...
public class ArrayListMod extends Module {
    ...
    private final BooleanSetting partialGlow = new BooleanSetting("Partial Glow", false);
    ...
    private final MultipleBoolSetting fontSettings = new MultipleBoolSetting("Font Settings",
            ...
            new BooleanSetting("Big Font", false), minecraftFont);
    ...
    public ArrayListMod() {
        ...
        addSettings(importantModules, rectangle, partialGlow, textShadow, splicedName, fontSettings, height, animation,
    ...
    public void getModulesAndSort() {
        if (modules == null || ModuleCollection.reloadModules) {
            List<Class<? extends Module>> hiddenModules = NIXWARE.INSTANCE.getModuleCollection().getHiddenModules();
            List<Module> moduleList = NIXWARE.INSTANCE.getModuleCollection().getModules();
        ...
        modules.sort(Comparator.<Module>comparingDouble(m -> {
            String name = HUDMod.get(m.getSplicedName() + (m.hasMode() ? " " + m.getSuffix() : ""));
            return font.getStringWidth(applyText(name));
        }).reversed());
    }

    public Dragging arraylistDrag = NIXWARE.INSTANCE.createDrag(this, "arraylist", 2, 1);
    ...
    public void onShaderEvent(ShaderEvent e) {
        ...
        for (Module module : modules) {
            ...
            String displayText = HUDMod.get(module.getSplicedName() + (module.hasMode() ? " §7" + module.getSuffix() : ""));
    ...
    public void onRender2DEvent(Render2DEvent e) {
        ...
        for (Module module : modules) {
            ...
            String displayText = HUDMod.get(module.getSplicedName() + (module.hasMode() ? (module.getCategory().equals(Category.EXPLOIT) ? " §c" : " §7") + module.getSuffix() : ""));
            displayText = applyText(displayText);
            float textWidth = font.getStringWidth(displayText);
            ...
            switch (rectangle.getMode()) {
                ...
                case "Outline":
                    if (count != 0) {
                        String modText = applyText(HUDMod.get(lastModule.getSplicedName() + (lastModule.hasMode() ? " " + lastModule.getSuffix() : "")));
    ...
    private AbstractFontRenderer getFont() {
        boolean BigFont = fontSettings.getSetting("Big Font").isEnabled();
        ...
        return BigFont ? bigFont20 : bigFont18;
    }
