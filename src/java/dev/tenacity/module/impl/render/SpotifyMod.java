...
public class SpotifyMod extends Module {
    ...
    private final Dragging drag = NIXWARE.INSTANCE.createDrag(this, "spotify", 5, 150);
            ...
            if (event.getBloomOptions().getSetting("Spotify").isEnabled()) {
                ...
                switch (backgroundColor.getMode()) {
                    case "Average":
                        ...
                        RoundedUtil.drawGradientVertical(x + (albumCoverSize - 15), y, playerWidth + 15, height, 0, color2, imageColor);
                        break;
                    case "Spotify Grey":
                        RoundedUtil.drawRound(x + (albumCoverSize - 15), y, playerWidth + 15, height, 0, new Color(0,0,0,128));
                        RoundedUtil.drawRound(x + (albumCoverSize - 15), y, playerWidth + 15, height, 0, new Color(255,255,255,15));
                        break;
                    case "Sync":
                        Pair<Color, Color> colors = HUDMod.getClientColors();
                        RoundedUtil.drawGradientCornerLR(x + (albumCoverSize - 15), y, playerWidth + 15, height, 0, colors.getFirst(), colors.getSecond());
                        break;
                }
                ...
                if (currentAlbumCover != null && downloadedCover) {
                    ...
                    RoundedUtil.drawRoundTextured(x, y, albumCoverSize, albumCoverSize, 0, 1);
                }

            } else {
                RoundedUtil.drawRound(x, y, playerWidth + (albumCoverSize), height, 0, Color.BLACK);
            }
        }
    }
    ...
    public void onRender2DEvent(Render2DEvent event) {
        ...
        switch (backgroundColor.getMode()) {
            ...
            case "Spotify Grey":
                RoundedUtil.drawRound(x + (albumCoverSize - 15), y, playerWidth + 15, height, 0, new Color(0,0,0,128));
                RoundedUtil.drawRound(x + (albumCoverSize - 15), y, playerWidth + 15, height, 0, new Color(25,25,25,255));
                break;
            case "Sync":
                Pair<Color, Color> colors = HUDMod.getClientColors();
                RoundedUtil.drawGradientCornerLR(x + (albumCoverSize - 15), y, playerWidth + 15, height, 0, colors.getFirst(), colors.getSecond());
        ...
        RenderUtil.scissor(x + albumCoverSize, y, playerWidth, height, () -> {
            ...
            mc.fontRendererObj.drawString(currentTrack.getName(), needsToScrollTrack ? trackX : x + albumCoverSize + 3, y + 3, -1);

            float artistX = (float) (((x + albumCoverSize) - tenacityFont18.getStringWidth(artistsDisplay.toString())) +
                    ((mc.fontRendererObj.getStringWidth(artistsDisplay.toString()) + playerWidth) * scrollArtist.getLinearOutput()));

            mc.fontRendererObj.drawString(artistsDisplay.toString(), needsToScrollArtist ? artistX : x + albumCoverSize + 4, y + 17, -1);
        });

        //Draw time left on song
        mc.fontRendererObj.drawString(trackRemaining, x + width - (mc.fontRendererObj.getStringWidth(trackRemaining) + 2),
                y + height - (mc.fontRendererObj.getHeight() + 2), -1);
        ...
        RoundedUtil.drawRound(x + albumCoverSize + 5, y + height - (progressBarHeight + 4.5f), progressBarWidth, progressBarHeight, 0, progressBackground);
        RoundedUtil.drawRound(x + albumCoverSize + 5, y + height - (progressBarHeight + 4.5f), progress, progressBarHeight, 0, progressColor);
        ...
        RoundedUtil.drawRound(x + albumCoverSize / 2f - 22.5f, y + albumCoverSize / 2f - 22.5f, 45, 45, 0, ColorUtil.applyOpacity(circleColor, (float) (.47 * playAnimation.getOutput().floatValue())));
