...
public class LoginPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        for (ActionButton actionButton : actionButtons) {
            ...
            actionButton.setClickAction(() -> {
                switch (actionButton.getName()) {
                    case "Login":
                        NIXWARE.INSTANCE.getAltManager().getUtils().login(textFields.get(0), textFields.get(1));
                        resetTextFields();
                        NIXWARE.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                    case "Add":
                        TextField username = textFields.get(0);
                        ...
                        break;
                    case "Gen Cracked":
                        NIXWARE.INSTANCE.getAltManager().getUtils().loginWithString(generator.generate(8), "", false);
                        NIXWARE.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                }
            });
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("nixware/mc.png"));
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (hoveringMicrosoft && button == 0) {
            ...
            NIXWARE.INSTANCE.getAltManager().getUtils().microsoftLoginAsync(email, password);
            resetTextFields();
        }
