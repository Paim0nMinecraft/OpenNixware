...
public class CategoryPanel implements Screen {
    ...
    public void initGui() {
        if (moduleRects == null) {
            ...
            for (Module module : NIXWARE.INSTANCE.getModuleCollection().getModulesInCategory(category).stream().sorted(Comparator.comparing(Module::getName)).collect(Collectors.toList())) {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (ClickGUIMod.outlineAccent.isEnabled()) {
            if (theme.equals(Theme.DEV)) {
                Color temp = clientFirst;
                clientFirst = clientSecond;
                clientSecond = temp;
            }
            if (DropdownClickGUI.gradient) {
                RoundedUtil.drawGradientVertical(x - .75f, y - .5f, rectWidth + 1.5f, realHeight + categoryRectHeight + 1.5f, 1, clientFirst, clientSecond);
            } else {
                RoundedUtil.drawRound(x - .75f, y - .5f, rectWidth + 1.5f, realHeight + categoryRectHeight + 1.5f, 1, clientFirst);
            }
        } else {
            RoundedUtil.drawRound(x - .75f, y - .5f, rectWidth + 1.5f, realHeight + categoryRectHeight + 1.5f, 1,
                    ColorUtil.tripleColor(20, alphaValue));
            if (!ClickGUIMod.transparent.isEnabled()) {
                Gui.drawRect2(x, y + categoryRectHeight, rectWidth, 3, clientFirst.getRGB());
            }
            if (DropdownClickGUI.gradient) {
                RoundedUtil.drawGradientVertical(x + 1, y + categoryRectHeight + 1, rectWidth - 2, realHeight - 2, 1, clientFirst, clientSecond);
            } else {
                RoundedUtil.drawRound(x + .8f, y + categoryRectHeight + .8f, rectWidth - 1.6f, realHeight - 1.6f, 1, clientFirst);
            }
        ...
        RoundedUtil.drawRound(x + 1, y + categoryRectHeight + 5, rectWidth - 2, realHeight - 6, 1, Color.BLACK);
        ...
        RenderUtil.resetColor();
        float textWidth = csgoFont22.getStringWidth(category.name + " ") / 2f;
        ...
        RenderUtil.resetColor();
        csgoFont22.drawString(category.name, x + ((rectWidth / 2f - textWidth) - (iconFont20.getStringWidth(category.icon) / 2f)),
                y + csgoFont22.getMiddleOfBox(categoryRectHeight), textColor);
    ...
    public void renderEffects() {
        ...
        boolean glow = Shader.glowOptions.getSetting("ClickGui").isEnabled();

        if (!ClickGUIMod.outlineAccent.isEnabled()) {
            RoundedUtil.drawRound(x - .75f, y - .5f, rectWidth + 1.5f, allowedHeight + categoryRectHeight + 1.5f, 1, Color.BLACK);
            return;
        }
        ...
        if (DropdownClickGUI.gradient && glow && ClickGUIMod.outlineAccent.isEnabled()) {
            ...
            RoundedUtil.drawGradientVertical(x - .75f, y - .5f, rectWidth + 1.5f, allowedHeight + categoryRectHeight + 1.5f, 1,
                    clientFirst, clientSecond);

        } else {
            RoundedUtil.drawRound(x - .75f, y - .5f, rectWidth + 1.5f, allowedHeight + categoryRectHeight + 1.5f, 1, (glow && ClickGUIMod.outlineAccent.isEnabled()) ? clientFirst :
    ...
    public List<ModuleRect> getModuleRects() {
        ...
        String search = NIXWARE.INSTANCE.getSearchBar().getSearchField().getText();
