...
public class KeybindComponent extends SettingComponent<KeybindSetting> {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        float fullTextWidth = csgoFont16.getStringWidth("Bind: §l" + bind);

        float startX = x + width / 2f - fullTextWidth / 2f;
        float startY = y + csgoFont16.getMiddleOfBox(height);

        boolean hovering = HoveringUtil.isHovering(startX - 3, startY - 2, fullTextWidth + 6, csgoFont16.getHeight() + 4, mouseX, mouseY);
        hoverAnimation.setDirection(hovering ? Direction.FORWARDS : Direction.BACKWARDS);

        Color rectColor = ColorUtil.brighter(settingRectColor, .7f - (.25f * hoverAnimation.getOutput().floatValue()));
        RoundedUtil.drawRound(startX - 3, startY - 2, fullTextWidth + 6, csgoFont16.getHeight() + 4, 4, rectColor);


        csgoFont16.drawCenteredString("Bind: §l" + bind, x + width /2f, y + csgoFont16.getMiddleOfBox(height), textColor);
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        float textWidth = csgoFont18.getStringWidth(text);
        float startX = x + width / 2f - textWidth / 2f;
        float startY = y + csgoFont18.getMiddleOfBox(height);
        float rectHeight = csgoFont18.getHeight() + 4;

        boolean hovering = HoveringUtil.isHovering(startX - 3, startY - 2, textWidth + 6, csgoFont18.getHeight() + 4, mouseX, mouseY);
