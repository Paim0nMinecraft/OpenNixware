...
public class ModeComponent extends SettingComponent<ModeSetting> {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        csgoFont16.drawString(modeSetting.name, boxX + 1, y + 3, textColor);

        csgoFont16.drawString(modeSetting.getMode(), boxX + 5, boxY + csgoFont16.getMiddleOfBox(boxHeight), textColor);
        ...
        if(themeSetting){
            ...
            float spacing = 3;
            RoundedUtil.drawRound(boxX + 7.5f + csgoFont16.getStringWidth(modeSetting.getMode()), boxY + middleOfRect,
                    width, height, 2.25f, themeColors.getFirst());
            RoundedUtil.drawRound(boxX + 7.5f + csgoFont16.getStringWidth(modeSetting.getMode()) + (spacing + width), boxY + middleOfRect,
        ...
        if (opened || !openAnimation.isDone()) {
            ...
            for (String mode : modeSetting.modes) {
                ...
                RenderUtil.resetColor();
                csgoFont16.drawString(mode, modeX + 5,
                        modeY + ((csgoFont16.getMiddleOfBox(rectHeight) + (rectHeight * rectCount)) * openAnimation.getOutput().floatValue()),
