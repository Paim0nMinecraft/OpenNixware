...
public class SideGUIHotbar implements Screen {
    ...
    public final TextField searchField = new TextField(csgoFont20);
    ...
    @Getter
    private final CarouselButtons carouselButtons = new CarouselButtons("Configs","About");
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        csgoFont32.drawString(NIXWARE.NAME, x + 9.5f, y + csgoFont32.getMiddleOfBox(height), textColor);
        csgoFont18.drawString(NIXWARE.VERSION, x + 12F + csgoFont32.getStringWidth(NIXWARE.NAME) - 2,
                y + csgoFont32.getMiddleOfBox(height) - 2.5f, ColorUtil.applyOpacity(textColor, .5f));
        ...
        if (!searching) {
            ...
            csgoFont14.drawString(String.valueOf(Math.round(refreshTimer.getTime() / 1000f)), refreshButton.getX() - 15,
                    y + csgoFont14.getMiddleOfBox(height), ColorUtil.applyOpacity(-1, refreshText.getOutput().floatValue()));
            ...
            refreshButton.setClickAction(() -> Multithreading.runAsync(() -> NIXWARE.INSTANCE.getCloudDataManager().refreshData()));

            boolean refreshing = NIXWARE.INSTANCE.getCloudDataManager().isRefreshing();
            if (refreshing) {
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (!NIXWARE.INSTANCE.getCloudDataManager().isRefreshing() && refreshTimer.hasTimeElapsed(12000)) {
