...
public class EditForm extends Form {
    ...
    private final TextField descriptionField = new TextField(csgoFont18);
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        float infoX = getX() + csgoFont40.getStringWidth(getTitle()) + 15;
        float infoY = getY() + 3 + csgoFont16.getMiddleOfBox(csgoFont40.getHeight()) + 2;

        csgoFont16.drawString("Choose to either update or delete your " + type.toLowerCase(),
                infoX, infoY, ColorUtil.applyOpacity(getTextColor(), .5f));

        csgoFont16.drawString(data.getName(), getX() + 5, getY() + 5.5f + csgoFont40.getHeight(), getAccentColor());
        ...
        csgoFont26.drawString("Update", updateX + 5, updateY + 3, getTextColor());
        ...
        descriptionField.setFont(csgoFont16);
        ...
        csgoFont18.drawString("Description", descriptionField.getXPosition(),
                descriptionField.getYPosition() - (csgoFont18.getHeight() + 5), getTextColor());
        ...
        if (error) {
            csgoFont16.drawCenteredStringWithShadow("Error please fill out the required fields",
                    updateButton.getX() + updateButton.getWidth() / 2f, updateButton.getY() - (csgoFont16.getHeight() + 5),
                    NIXWARE.INSTANCE.getSideGui().getRedBadColor().getRGB());
        }

        if (script) {
            csgoFont16.drawString("Update the script with one of your local scripts",
                    updateX + 10 + csgoFont26.getStringWidth("Update"),
                    updateY + 4 + csgoFont16.getMiddleOfBox(csgoFont26.getHeight()), ColorUtil.applyOpacity(getTextColor(), .5f));
            ...
            csgoFont16.drawString("Update the config with your current settings",
                    updateX + 10 + csgoFont26.getStringWidth("Update"),
                    updateY + 4 + csgoFont16.getMiddleOfBox(csgoFont26.getHeight()), ColorUtil.applyOpacity(getTextColor(), .5f));
        }
        ...
        csgoFont26.drawString("Delete", deleteX + 5, deleteY + 3, getTextColor());
        iconFont26.drawString(FontUtil.TRASH, deleteX + 10 + csgoFont26.getStringWidth("Delete"), deleteY + 4.5f, getTextColor());

        csgoFont16.drawString("Delete the " + type.toLowerCase() + " from the", deleteX + 5, deleteY + 22, ColorUtil.applyOpacity(getTextColor(), .5f));
        csgoFont16.drawString("cloud", deleteX + 5, deleteY + 22 + csgoFont16.getHeight() + 3, ColorUtil.applyOpacity(getTextColor(), .5f));
        ...
        deleteButton.setColor(NIXWARE.INSTANCE.getSideGui().getRedBadColor());
        deleteButton.setClickAction(() -> {
            Multithreading.runAsync(() -> {
                ...
                NIXWARE.INSTANCE.getSideGui().displayForm(null);
                NIXWARE.INSTANCE.getCloudDataManager().refreshData();
            });
        });
