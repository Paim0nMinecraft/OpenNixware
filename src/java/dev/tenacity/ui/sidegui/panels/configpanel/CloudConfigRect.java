...
public class CloudConfigRect implements Screen {
    ...
    public CloudConfigRect(CloudConfig config) {
        ...
        NIXWARE.INSTANCE.getSideGui().getTooltips().add(hoverInformation);
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        csgoFont26.drawString(config.getName(), x + 3, y + 3, textColor);

        float yOffset = compact ? 2.5f : 2;

        csgoFont16.drawString(config.getAuthor(), x + 3, y + yOffset + csgoFont32.getHeight(), accentColor);

        csgoFont16.drawString(CloudDataUtils.getLastEditedTime(config.getLastUpdated()),
                x + 5 + csgoFont16.getStringWidth(config.getAuthor()), y + yOffset + csgoFont32.getHeight(), ColorUtil.applyOpacity(textColor, .5f));
        ...
        if (!compact) {
            ...
            csgoFont16.drawWrappedText(config.getDescription(), x + 3,
                    y + 6 + csgoFont32.getHeight() + csgoFont16.getHeight(),
                ...
                switch (iconButton.getIcon()) {
                    case FontUtil.LOAD:
                        Multithreading.runAsync(() -> {
                            ...
                            if (NIXWARE.INSTANCE.getConfigManager().loadConfig(loadData, false)) {
                    ...
                    case FontUtil.SAVE:
                        Multithreading.runAsync(() -> {
                            ...
                            if (NIXWARE.INSTANCE.getConfigManager().saveConfig(name, saveData)) {
                            ...
                            NIXWARE.INSTANCE.getSideGui().getTooltips().clear();
                            NIXWARE.INSTANCE.getSideGui().getConfigPanel().setRefresh(true);
                        });
                        break;
                    case FontUtil.EDIT:
                        Form form = NIXWARE.INSTANCE.getSideGui().displayForm("Edit Config");
                        ((EditForm) form).setup(config, false);
                        form.setUploadAction((fileName, updatedDescription) -> {
                            Multithreading.runAsync(() -> {

                                String data = NIXWARE.INSTANCE.getConfigManager().serialize();
        ...
        boolean hoveringInfo = SideGUI.isHovering(getX() + 3, getY() + getHeight() - (csgoFont14.getHeight() + 3),
                iconFont20.getStringWidth(FontUtil.INFO) + 2 + csgoFont14.getStringWidth("Hover for more information"),
                csgoFont14.getHeight() + 3, mouseX, mouseY);
        ...
        csgoFont14.drawString("Hover for more information", getX() + 5 + iconFont16.getStringWidth(FontUtil.INFO),
                getY() + getHeight() - (csgoFont14.getHeight() + 3), ColorUtil.applyOpacity(textColor, .35f + additionalAlpha));
