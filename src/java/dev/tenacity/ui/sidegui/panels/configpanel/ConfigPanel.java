...
public class ConfigPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (voteRefreshTimer.hasTimeElapsed(30000)) {
            Multithreading.runAsync(() -> NIXWARE.INSTANCE.getCloudDataManager().refreshVotes());
            voteRefreshTimer.reset();
        }


        NIXWARE.INSTANCE.getCloudDataManager().applyVotes();

        csgoFont40.drawString("Configs", getX() + 8, getY() + 8, getTextColor());
        ...
        for (ActionButton button : actionButtons) {
            ...
            button.setClickAction(() -> {
                switch (button.getName()) {
                    case "Upload config":
                        Form uploadForm = NIXWARE.INSTANCE.getSideGui().displayForm("Upload Config");
                        uploadForm.setTriUploadAction((name, description, server) -> {
                            Multithreading.runAsync(() -> {
                                String fileData = NIXWARE.INSTANCE.getConfigManager().serialize();
                    ...
                    case "Save current config":
                        Form saveForm = NIXWARE.INSTANCE.getSideGui().displayForm("Save Config");
                        saveForm.setUploadAction((name, description) -> {
                            Multithreading.runAsync(() -> {
                                NIXWARE.INSTANCE.getConfigManager().saveConfig(name);
                                NIXWARE.INSTANCE.getCloudDataManager().refreshData();
                            });
                        });
                        break;
        ...
        switch (carouselButtons.getCurrentButton()) {
            case "Cloud":
                ...
                if (!sortingSelection.equals(sorting.getSelection())) {
                    Multithreading.runAsync(() -> NIXWARE.INSTANCE.getCloudDataManager().refreshData());
                    sortingSelection = sorting.getSelection();
                }

                if (NIXWARE.INSTANCE.getCloudDataManager().isRefreshing())
                    return;
                ...
                for (CloudConfigRect cloudConfigRect : cloudConfigRects) {
                    ...
                    if (filterAuthor) {
                        ...
                        if (!NIXWARE.INSTANCE.getIntentAccount().username.equals(configAuthor)) continue;
                    }

                    if (filterVersion && !cloudConfigRect.getConfig().getVersion().equals(NIXWARE.VERSION)) continue;
    ...
    public void refresh() {
        ...
        for (CloudConfig cloudConfig : NIXWARE.INSTANCE.getCloudDataManager().getCloudConfigs()) {
    ...
    public Comparator<CloudConfigRect> relevanceSorting() {
        return Comparator.<CloudConfigRect>comparingInt(configRect -> {
            ...
            versionScore = cloudConfig.getVersion().equals(NIXWARE.VERSION) ? 100 : 0;
