...
public class LocalConfigRect implements Screen {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (bfa != null) {
            csgoFont16.drawString(CloudDataUtils.getLastEditedTime(String.valueOf(bfa.lastModifiedTime().toMillis() / 1000)),
                    x + 4, y + 2.5f + csgoFont32.getHeight(),
        ...
        for (IconButton button : buttons) {
            ...
            button.setClickAction(() -> {
                switch (button.getIcon()) {
                    ...
                    case FontUtil.SAVE:
                        Multithreading.runAsync(() -> {
                            String saveData = NIXWARE.INSTANCE.getConfigManager().serialize();

                            if (NIXWARE.INSTANCE.getConfigManager().saveConfig(config.getName(), saveData)) {
