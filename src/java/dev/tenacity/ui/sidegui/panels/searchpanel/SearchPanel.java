...
public class SearchPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        csgoFont18.drawString("Press ESC to return to the menu", getX() + 8, getY() + 8 + csgoFont40.getHeight() + 2, ColorUtil.applyOpacity(getTextColor(), .3f));

        csgoFont40.drawString("Search Results", getX() + 8, getY() + 8, getTextColor());
        ...
        String search = NIXWARE.INSTANCE.getSideGui().getHotbar().searchField.getText();
        ...
        if (!searchTypeHold.equals(searchType)) {
            switch (searchType) {
                case "Configs":
                    ...
                    cloudConfigRects.addAll(NIXWARE.INSTANCE.getSideGui().getConfigPanel().getCloudConfigRects());
                    break;
                case "Scripts":
                    cloudConfigRects.clear();
                    break;
