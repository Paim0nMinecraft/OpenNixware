...
public class VoteRect implements Screen {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        Color greenColor = NIXWARE.INSTANCE.getSideGui().getGreenEnabledColor();
        ...
        RenderUtil.scissorStart(x, y + height / 2f - (csgoFont16.getHeight() / 2f + 1.5), width, csgoFont16.getHeight() + 3);
        drawAnimatedVote(totalVotes, voteColor);
        RenderUtil.scissorEnd();

        //      csgoFont16.drawCenteredString(s, x + width / 2f, y + csgoFont16.getMiddleOfBox(height), ColorUtil.applyOpacity(voteColor, alpha));
    }
        ...
        if (lastVote != vote) {
            ...
            if (voteAnimation == null) {
                voteAnimation = new DecelerateAnimation(Math.min(500, 200 * Math.abs(additionalVotes)),
                        additionalVotes * (csgoFont16.getHeight() + 3));
            }
            ...
            if (additionalVotes > 0) {
                ...
                for (int i = lastVote; i < (vote + 1); i++) {
                    int additionalY = count * (csgoFont16.getHeight() + 3);

                    float voteY = y + csgoFont16.getMiddleOfBox(height) + additionalY;
                    csgoFont16.drawCenteredString(String.valueOf(i), x + width / 2f, voteY - (animation), ColorUtil.applyOpacity(color, alpha));
                    count++;
                }
            ...
            csgoFont16.drawCenteredString(String.valueOf(vote), x + width / 2f,
                    y + csgoFont16.getMiddleOfBox(height), ColorUtil.applyOpacity(color, alpha));
        }
