...
public class RenderUtil implements Utils {
    ...
    public static void roundedRectangle(double x, double y, double width, double height, double radius, Color color) {
        RoundedUtil.drawRound((float)x,  (float)y, (float) width,  (float)height,(float)radius, color);
    }
    ...
    public static void drawBorderedRectNovo(float x, float y, float width, float height, final float outlineThickness, int rectColor, int outlineColor) {
        Gui.drawRect2(x, y, width, height, rectColor);
        glEnable(GL_LINE_SMOOTH);
        color(outlineColor);
