...
public class  GuiIngame extends Gui implements Utils {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        NIXWARE.INSTANCE.getEventProtocol().handleEvent(new PreRenderEvent());


        Shader postProcessing = (Shader) NIXWARE.INSTANCE.getModuleCollection().get(Shader.class);
        postProcessing.blurScreen();


        NIXWARE.INSTANCE.getEventProtocol().handleEvent(new Render2DEvent(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight()));
        NotificationsMod notif = NIXWARE.INSTANCE.getModuleCollection().getModule(NotificationsMod.class);
        if (notif.isEnabled()) {
            notif.render();
        }
        ...
        if (scoreobjective1 != null && NIXWARE.INSTANCE.isEnabled(ScoreboardMod.class)) {
    ...
    public void renderExpBar(ScaledResolution scaledRes, int x) {
        ...
        if (mc.thePlayer.xpBarCap() > 0) {
            ...
            if (mc.thePlayer.experienceLevel > 0) {
                ...
                float length = csgoFont12.getStringWidth(str);
                expAnim.animate(mc.thePlayer.experience, 18);
                RoundedUtil.drawRound(x + length + 2, l, 182 - length - 2, 5, 0, new Color(255, 255, 255,15));
                RoundedUtil.drawRound(x + length + 2, l, expAnim.getOutput() * (182 - length - 2), 5, 0, new Color(0, 0, 0,128));
                csgoFont12.drawString(str, x, l - csgoFont12.getHeight() / 2f + 2.5f, -1);
            } else {
                RoundedUtil.drawRound(x, l, 182, 5, 0, new Color(255, 255, 255,15));
                RoundedUtil.drawRound(x, l, mc.thePlayer.experience * 182, 5, 0, new Color(0, 0, 0,128));
            }
        }
    ...
    public void renderScoreboardBlur(ScaledResolution scaledRes) {
        ...
        if (objective != null && NIXWARE.INSTANCE.isEnabled(ScoreboardMod.class)) {
    ...
    private void renderPlayerStats2(ScaledResolution scaledRes) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            EntityPlayer entityplayer = (EntityPlayer) this.mc.getRenderViewEntity();
            int i = MathHelper.ceiling_float_int(entityplayer.getHealth());
    ...
    public AbstractFontRenderer getScoreboardFontRenderer() {
        AbstractFontRenderer font = dev.tenacity.utils.font.hanabi.FontUtil.getFromCustomFont(FontUtil.bigFont18);
        return ScoreboardMod.customFont.isEnabled() ? font : this.mc.fontRendererObj;
    }
